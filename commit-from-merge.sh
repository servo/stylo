#!/bin/sh
# Usage: commit-from-merge.sh <merge commit> [extra git-commit(1) arguments ...]
...
author_name_email=$(git log -n1 --pretty='%aN <%aE>' "$merge_commit"\^2)
committer_date=$(git log -n1 --pretty='%cd' "$merge_commit")
