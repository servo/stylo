...
/// If a PropertyDeclarationIdSet is provided, it will be populated with the animated properties.
fn has_animated_properties(
    keyframes: &[Arc<Locked<Keyframe>>],
    guard: &SharedRwLockReadGuard,
    mut properties_changed: Option<&mut PropertyDeclarationIdSet>,
) -> bool {
    ...
    for keyframe in keyframes {
        ...
        for declaration in block.normal_declaration_iter() {
            ...
            if let Some(ref mut properties_changed) = properties_changed {
                properties_changed.insert(declaration_id);
            } else {
                return true;
            }
        }
...
impl KeyframesAnimation {
    ...
    pub fn from_keyframes(
        ...
        #[cfg(feature = "servo")]
        let properties_changed = Some(&mut result.properties_changed);
        #[cfg(feature = "gecko")]
        let properties_changed = None;

        if keyframes.is_empty() || !has_animated_properties(keyframes, guard, properties_changed) {
            return result;
        }
