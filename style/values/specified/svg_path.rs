    ...
    fn parse_curveto(&mut self, by_to: ByTo) -> Result<(), ()> {
        if by_to.is_abs() {
            parse_arguments!(self, CubicCurve, [
                control1 => parse_control_point_abs, control2 => parse_control_point_abs, point => parse_command_point_abs
            ])
        } else {
            parse_arguments!(self, CubicCurve, [
                control1 => parse_control_point_rel, control2 => parse_control_point_rel, point => parse_command_point_rel
            ])
        }
    }
    ...
    fn parse_smooth_curveto(&mut self, by_to: ByTo) -> Result<(), ()> {
        if by_to.is_abs() {
            parse_arguments!(self, SmoothCubic, [
                control2 => parse_control_point_abs, point => parse_command_point_abs
            ])
        } else {
            parse_arguments!(self, SmoothCubic, [
                control2 => parse_control_point_rel, point => parse_command_point_rel
            ])
        }
    }
    ...
    fn parse_quadratic_bezier_curveto(&mut self, by_to: ByTo) -> Result<(), ()> {
        if by_to.is_abs() {
            parse_arguments!(self, QuadCurve, [
                control1 => parse_control_point_abs, point => parse_command_point_abs
            ])
        } else {
            parse_arguments!(self, QuadCurve, [
                control1 => parse_control_point_rel, point => parse_command_point_rel
            ])
        }
    }
...
fn parse_control_point_abs(
    ...
    let coord = parse_coord(iter)?;
    Ok(ControlPoint::Position(coord.into()))
}
