    ...
    fn to_shmem(&self, builder: &mut SharedMemoryBuilder) -> Result<Self> {
        let dest_vec = unsafe {
            if self.spilled() {
                ...
                // buffer.
                let dest = to_shmem_slice(self.iter(), builder)? as *mut T;
                smallvec::SmallVec::from_raw_parts(dest, self.len(), self.len())
            } else {
    ...
    servo_arc::Arc<B>: ToShmem,
{
    fn to_shmem(&self, builder: &mut SharedMemoryBuilder) -> Result<Self> {
        Ok(ManuallyDrop::new(match self.borrow() {
            servo_arc::ArcUnionBorrow::First(first) => Self::from_first(ManuallyDrop::into_inner(
                first.with_arc(|a| a.to_shmem(builder))?,
            )),
            servo_arc::ArcUnionBorrow::Second(second) => Self::from_second(ManuallyDrop::into_inner(
...
impl<T: ToShmem> ToShmem for servo_arc::Arc<T> {
    fn to_shmem(&self, builder: &mut SharedMemoryBuilder) -> Result<Self> {
        ...
        unsafe {
            let static_arc = servo_arc::Arc::new_static(
...
impl ToShmem for smallbitvec::SmallBitVec {
    fn to_shmem(&self, builder: &mut SharedMemoryBuilder) -> Result<Self> {
        ...
        Ok(ManuallyDrop::new(unsafe {
            smallbitvec::SmallBitVec::from_storage(storage)
        }))
    }
}
